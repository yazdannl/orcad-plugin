export const SETUP_GUIDANCE = Object.freeze({
  firstRun: 'First Code-mode CAD run may install build123d and OCP via Orca’s bundled uv. It can download hundreds of MB, needs network and write access, and may take time. Gridfinity object mode separately provisions a pinned official OpenSCAD 2023-or-newer build into the per-user cache at startup; its first download may also take time.',
  recovery: 'If setup or a build fails, check network and write access, reopen the Plugins dialog or restart OrcaSlicer, then retry. The error may be setup, automatic OpenSCAD setup, or code.',
