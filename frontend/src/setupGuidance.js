export const SETUP_GUIDANCE = Object.freeze({
  firstRun: 'First CAD run may install build123d and OCP via Orca’s bundled uv. It can download hundreds of MB, needs network and write access, and may take time.',
  recovery: 'If setup or a build fails, check network and write access, reopen the Plugins dialog or restart OrcaSlicer, then retry. The error may be setup or code.',
  readiness: 'Bridge ready only means the host messaging API is available. CAD/model readiness is checked by a preview or export; there is no separate dependency probe.',
